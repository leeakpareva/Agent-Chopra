...
class TradingDashboard:
    ...
    def get_positions(self):
        """Get current positions with enhanced debugging and fallback"""
        try:
            ...
            if positions:
                ...
                # Add sample data for testing if no real positions
                sample_positions = [
                    {
                        'symbol': 'AAPL', 'Symbol': 'AAPL', 'Quantity': 10,
                        'Avg Entry': 150.00, 'Current Price': 155.50, 'Market Value': 1555.00,
                        'market_value': 1555.00, 'Unrealized P/L': 55.00, 'Unrealized P/L %': 3.67,
                        'pnl': 55.00, 'Side': 'long'
                    },
                    {
                        'symbol': 'MSFT', 'Symbol': 'MSFT', 'Quantity': 5,
                        'Avg Entry': 280.00, 'Current Price': 275.25, 'Market Value': 1376.25,
                        'market_value': 1376.25, 'Unrealized P/L': -23.75, 'Unrealized P/L %': -1.70,
                        'pnl': -23.75, 'Side': 'long'
                    },
                    {
                        'symbol': 'GOOGL', 'Symbol': 'GOOGL', 'Quantity': 3,
                        'Avg Entry': 120.00, 'Current Price': 128.50, 'Market Value': 385.50,
                        'market_value': 385.50, 'Unrealized P/L': 25.50, 'Unrealized P/L %': 7.08,
                        'pnl': 25.50, 'Side': 'long'
                    },
                    {
                        'symbol': 'TSLA', 'Symbol': 'TSLA', 'Quantity': 2,
                        'Avg Entry': 200.00, 'Current Price': 195.75, 'Market Value': 391.50,
                        'market_value': 391.50, 'Unrealized P/L': -8.50, 'Unrealized P/L %': -2.13,
                        'pnl': -8.50, 'Side': 'long'
                    },
                    {
                        'symbol': 'NVDA', 'Symbol': 'NVDA', 'Quantity': 4,
                        'Avg Entry': 450.00, 'Current Price': 485.20, 'Market Value': 1940.80,
                        'market_value': 1940.80, 'Unrealized P/L': 140.80, 'Unrealized P/L %': 7.82,
                        'pnl': 140.80, 'Side': 'long'
                    }
                ]
                position_data = sample_positions
                st.info("📊 Displaying sample portfolio data - Connect live trading for real positions")
    ...
    def create_pnl_chart(self, positions_df):
        """Create enhanced P&L visualization with improved colors"""
        if positions_df.empty:
            return None

        # Enhanced color palette for better stock visualization
        stock_colors = [
            '#DC143C',  # Primary red
            '#FF6B6B',  # Light red
            '#4ECDC4',  # Teal
        ...
        # Enhanced pie chart for position distribution
        fig.add_trace(
            go.Pie(
                ...
                name="Market Value",
                marker=dict(
                    colors=stock_colors[:len(positions_df)],
        ...
        # Enhanced bar chart for P&L with gradient colors
        pnl_values = positions_df['Unrealized P/L']
        colors = []
        for value in pnl_values:
            if value > 0:
                # Green gradient for profits
                intensity = min(abs(value) / max(abs(pnl_values)) if max(abs(pnl_values)) > 0 else 0, 1)
                colors.append(f'rgba(76, 175, 80, {0.5 + intensity * 0.5})')
            else:
                # Red gradient for losses
                intensity = min(abs(value) / max(abs(pnl_values)) if max(abs(pnl_values)) > 0 else 0, 1)
                colors.append(f'rgba(244, 67, 54, {0.5 + intensity * 0.5})')
